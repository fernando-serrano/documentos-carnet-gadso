...
## Nuevo flujo SIGEMO - Galenius (modular)

Se agrego una base no monolitica para el nuevo flujo de certificados medicos:

- `sigemo_galenius/config.py`: carga y valida variables de entorno.
- `sigemo_galenius/selectors.py`: selectores UI centralizados de Galenius.
- `sigemo_galenius/login_flow.py`: login automatizado con Playwright y validacion post-login.
- `sigemo_galenius/logging_utils.py`: logging por corrida y eventos JSONL.
- `sigemo_galenius/pipeline.py`: esqueleto del pipeline de lote (descarga/compresion).
- `run_galenius_login.py`: entrypoint de la primera etapa (solo login).
- `run_galenius_login.bat`: launcher para Windows.
...
- `logs/galenius/runs/galenius_YYYYMMDD_HHMMSS/galenius_login.log`
- `logs/galenius/runs/galenius_YYYYMMDD_HHMMSS/events.jsonl`
